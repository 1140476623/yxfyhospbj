...
public class HttpsClient {
     ...
     * 连接超时时间
     */
    private static Integer CONNECTION_TIMEOUT = WxPayConfig.connectionTimeout;
    /**
     * 请求超时时间
     */
    private static Integer READ_TIMEOUT = WxPayConfig.readTimeout;
     ...
     * @return JSONObject
     */
    public static void httpsRequestReturnJSONObject(String requestUrl, String requestMethod, String postData) throws Exception {
        String s = HttpsClient.httpsRequestReturnString(requestUrl, requestMethod, postData);
    ...
    public static String httpsRequestReturnString(String requestUrl, String requestMethod, String postData) throws Exception {
        ...
        try {
            ...
            //设置编码
            httpsUrlConnection.setRequestProperty("Charsert", WxPayConstants.DEFAULT_CHARSET);

            //判断是否需要提交数据
            if (StringUtils.equals(requestMethod, HttpsClient.METHOD_POST) && StringUtils.isNotBlank(postData)) {
                //讲参数转换为字节提交
                byte[] bytes = postData.getBytes(WxPayConstants.DEFAULT_CHARSET);
                //设置头信息
                ...
                outputStream.write(postData.getBytes(WxPayConstants.DEFAULT_CHARSET));
                outputStream.flush();
                outputStream.close();
            } else {
                //开始连接
            ...
            response = WxPayUtils.getStreamString(httpsUrlConnection.getInputStream());
        } catch (Exception e) {
            throw new Exception();
        } finally {
